  ...
  return (
    <div className="flex-1 overflow-y-auto px-5 py-4">
      ...
      <div className="flex items-center justify-between mb-3">
        <h3 className="text-[10px] font-bold text-[var(--color-text-tertiary)] tracking-wider uppercase">
          Recent Sessions
        </h3>
        <button
      ...
      <div className="flex flex-col gap-0.5">
            ...
            >
              <div className="flex items-center gap-3 overflow-hidden">
                ...
                {editingId === session.id ? (
                  <input
                    ...
                    className="flex-1 bg-[var(--color-bg-dark)] text-[var(--color-text-primary)] text-xs px-2 py-1 rounded outline-none border border-[var(--color-accent)] w-full"
                  />
                ) : (
                ...
                {isProcessing && (
                  <span
                    className="flex-shrink-0 w-2 h-2 rounded-full bg-[var(--color-accent)]"
              ...
              {editingId !== session.id && (
                <div className={`flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity ${isActive ? 'opacity-100' : ''}`}>
                  <button onClick={(e) => startEditing(e, session)} className="p-1 text-[var(--color-text-tertiary)] hover:text-[var(--color-text-primary)] rounded">
                    <Edit2 size={12} />
                  </button>
                  <button onClick={(e) => handleDelete(e, session.id)} className="p-1 text-[var(--color-text-tertiary)] hover:text-[var(--color-error)] rounded">
                    <Trash2 size={12} />
                  </button>
                </div>
              )}
        ...
        {sessions.length === 0 && (
          <div className="text-center py-6 text-xs text-[var(--color-text-disabled)]">
            No active sessions.
          </div>
        )}
      </div>
