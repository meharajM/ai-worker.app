...
export function OpenAISettings({
    ...
    return (
        ...
        >
            {/* API Key */}
            <div>
                <label className="block text-xs text-[var(--color-text-tertiary)] mb-1">API Key</label>
                <div className="flex gap-2">
                    <input
                        ...
                        className="flex-1 bg-[var(--color-bg-dark)] border border-[var(--color-border)] rounded-lg px-3 py-2 text-sm text-[var(--color-text-primary)] placeholder:text-[var(--color-text-disabled)] focus:border-[var(--color-border-hover)] focus:outline-none"
                    />
                    {!isOpenRouter && apiKey && (
                        <button
                            onClick={onRefresh}
                            disabled={modelsEndpointAvailable === false}
                            className="px-3 py-2 text-xs bg-[var(--color-accent-muted)] hover:bg-[var(--color-accent)]/20 text-[var(--color-accent)] rounded-lg font-medium transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
            ...
            <div>
                ...
                {models && models.length > 0 ? (
                    ...
                    <p className="text-xs text-[var(--color-text-disabled)] mt-1">Could not fetch models: {error}. Type model name manually.</p>
                ) : null}
            </div>
        </ProviderCard>
    )
