...
export function ChatView({ onClearChat }: ChatViewProps) {
  ...
  return (
    <div className="flex-1 flex flex-col overflow-hidden min-w-0">
      ...
      {messages.length > 0 && (
        <div className="flex justify-end px-4 py-2 border-b" style={{ borderColor: 'var(--color-border)' }}>
          <button
            onClick={handleClear}
            className="flex items-center gap-1.5 text-xs px-2 py-1 rounded transition-all hover:bg-[var(--color-error-muted)]"
