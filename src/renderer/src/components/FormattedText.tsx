...
const CodeBlock = ({ inline, className, children, ...props }: any) => {
  ...
  if (!inline && isSingleLine && isShort && isTextOrUnknown) {
      ...
      return (
          <div className="relative group/code my-1 inline-block align-middle">
               <div className={cn(
                   "flex items-center gap-2 bg-[var(--color-bg-surface)] border border-[var(--color-border)] rounded-md px-2 py-1",
                   isSuperShort ? "px-1.5 py-0.5" : ""
               )}>
                   <code className="font-mono text-xs text-[var(--color-accent)] flex-1 break-all">
                       {codeContent}
                   </code>
                   {!isSuperShort && (
                       <button
                            onClick={handleCopy}
                            className="p-0.5 rounded hover:bg-[var(--color-bg-raised)] text-[var(--color-text-tertiary)] hover:text-[var(--color-text-primary)] transition-all opacity-0 group-hover/code:opacity-100"
                            title="Copy"
                        >
  ...
  if (!inline) {
    return (
      <div className="relative group/code my-4 rounded-xl bg-[var(--color-bg-surface)] border border-[var(--color-border)] overflow-hidden shadow-sm">
        {/* Header / Language Badge */}
        <div className="flex items-center justify-between px-3 py-2 bg-[var(--color-bg-elevated)] border-b border-[var(--color-border)]">
          <div className="flex items-center gap-2">
            <Terminal size={12} className="text-[var(--color-text-tertiary)]" />
            <span className="text-[11px] font-medium text-[var(--color-text-secondary)] font-mono tracking-wide">
              {language}
            </span>
          </div>
          <button
            onClick={handleCopy}
            className="p-1 rounded hover:bg-[var(--color-bg-surface)] text-[var(--color-text-tertiary)] hover:text-[var(--color-text-primary)] transition-all flex items-center gap-1.5"
            title="Copy code"
          >
  ...
  return (
    <code 
        className={cn(
            "px-1.5 py-0.5 rounded bg-[var(--color-bg-surface)] text-[var(--color-accent)] font-mono text-[0.9em]",
            className
        )} 
        {...props}
    >
...
export function FormattedText({ content, className = '' }: FormattedTextProps) {
  ...
  return (
    <div className={`prose max-w-none text-sm leading-relaxed ${className} select-text`}>
      <ReactMarkdown
        remarkPlugins={[remarkGfm]}
        components={{
          // Headers - clear separation
          ...
          h3: ({ children }) => <h3 className="text-[13px] font-bold text-[var(--color-text-primary)] mt-3 mb-1.5 uppercase tracking-wide">{children}</h3>,
          
          // Lists - proper spacing and bullets
          ul: ({ children }) => <ul className="list-disc pl-4 space-y-1 my-2 text-[var(--color-text-primary)]">{children}</ul>,
          ol: ({ children }) => <ol className="list-decimal pl-4 space-y-1 my-2 text-[var(--color-text-primary)]">{children}</ol>,
          ...
          p: ({ children }) => <p className="mb-2 last:mb-0 text-[var(--color-text-secondary)]">{children}</p>,
          blockquote: ({ children }) => <blockquote className="border-l-2 border-[var(--color-accent)] pl-4 italic text-[var(--color-text-tertiary)] my-2">{children}</blockquote>,
          ...
          thead: ({ children }) => <thead className="bg-[var(--color-bg-surface)]">{children}</thead>,
          ...
          td: ({ children }) => <td className="px-3 py-2 text-xs text-[var(--color-text-secondary)] whitespace-nowrap">{children}</td>,
