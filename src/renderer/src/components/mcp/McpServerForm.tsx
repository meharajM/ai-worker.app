...
export function McpServerForm({ editingServer, onSubmit, onCancel }: McpServerFormProps) {
    const [name, setName] = useState(editingServer?.name || '')
    const [serverType, setServerType] = useState<'stdio' | 'sse'>(editingServer?.type as any || 'stdio')
